...
```bash
git tag -f v0.5.1 >/dev/null 2>&1
PYTHONPATH=src python3 scripts/release/generate_changelog.py --tag v0.5.1 --output /tmp/perf-skill-release-notes.md
sed -n '1,40p' /tmp/perf-skill-release-notes.md
git tag -d v0.5.1 >/dev/null 2>&1
```
...
- `perf_skill-0.5.1.tar.gz`
- `perf_skill-0.5.1-py3-none-any.whl`
